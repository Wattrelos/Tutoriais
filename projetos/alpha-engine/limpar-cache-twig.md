# Como Limpar o Cache das Páginas Twig

O cache das páginas do Twig é compilado e salvo no servidor em `storage/cache/twig_slim/`. Como esses arquivos compilados são gerados pelo usuário do servidor web (`www-data`), o atalho Ctrl + F5 do navegador limpa apenas o cache do seu browser, enquanto o PHP continua entregando os templates antigos já compilados no disco.
...
Todas as suas alterações nos arquivos `.twig` já estarão 100% atualizadas e visíveis no painel!