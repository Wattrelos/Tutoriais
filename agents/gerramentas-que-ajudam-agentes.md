...
A IA tende a misturar convenções de escrita (PSR-12, CamelCase, snake_case) dependendo de como os dados de treinamento a influenciaram.
...
* **[PHP_CodeSniffer](https://github.com/squizlabs/PHP_CodeSniffer):** Inspeciona violações de estilo (`phpcs`) e aplica correções automáticas (`phpcbf`), servindo como portão de qualidade indispensável.
...
| **Padronização** | PHP-CS-Fixer / PHP_CodeSniffer | Executa na conclusão de cada tarefa | Elimina revisões manuais de estilo e regras PSR |
